...
- 첫 문서: `docs/01-installation/01_environment.md`
- 실습 예제: `ops/examples/`
...
```
1. Environment      -> docs/01-installation/
   └── Python, venv, pip 실행 환경

2. Language Basics  -> docs/02-language/
   ├── 변수, 타입, 조건문, 반복문, 함수
   └── 컬렉션, 스코프, 예외, 클래스

3. Standard Library -> docs/03-stdlib/
   ├── 모듈/패키지, 파일, JSON/YAML/CSV
   └── subprocess, logging

4. Tooling          -> docs/04-tooling/
   ├── pytest, typing, packaging
   └── ops/tools/pyproject.example.toml 기반 설정 예시

5. Practical Topics -> docs/05-automation/ + docs/06-web/
   ├── argparse CLI 자동화
   └── HTTP API, FastAPI 기초
```
